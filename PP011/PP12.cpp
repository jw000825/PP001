...
void init(void)
{
    glClearColor(0.3, 0.8, 0.9, 0.0);
    //glShadeModel(GL_FLAT);  //GL_SMOOTH
    //glEnable(GL_DEPTH_TEST);

    makeCheckImage();
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);  // glPixelStorei -> 픽셀 데이터를 패킹(정렬)

    glGenTextures(1, &texName);             // 텍스쳐 생성(GPU1번 슬롯, 이름은 texName)
